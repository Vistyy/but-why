...
export type SessionTranscriptDiscovery =
  | { readonly ok: true; readonly transcripts: readonly SessionTranscript[] }
  | { readonly ok: false; readonly reason: string };

export type SessionTranscriptIndexResult =
  | { readonly ok: true }
  | { readonly ok: false; readonly reason: string };

export type SessionTranscriptIndexOperation = (input: {
  readonly ownerId: string;
  readonly sessionRoot: string;
}) => Effect.Effect<SessionTranscriptIndexResult, RepositoryStorageError>;

export const openSessionTranscriptIndex = (dependencies: {
  readonly recordTranscripts: (input: {
    readonly ownerId: string;
    readonly transcripts: readonly SessionTranscript[];
  }) => Effect.Effect<void, RepositoryStorageError>;
}): SessionTranscriptIndexOperation => {
  const index = (input: {
    readonly ownerId: string;
    readonly sessionRoot: string;
  }): Effect.Effect<SessionTranscriptIndexResult, RepositoryStorageError> =>
    Effect.gen(function* () {
      const discovery = discoverSessionTranscripts(input.sessionRoot, input.ownerId);
      if (!discovery.ok) return { ok: false, reason: discovery.reason };
      yield* dependencies.recordTranscripts({
...
export const discoverSessionTranscripts = (
  ownerRoot: string,
  ownerId: string,
): SessionTranscriptDiscovery => {
