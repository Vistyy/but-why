...
const cancelResult = (taskId: PublicTaskId, result: TaskCancellationResult): CliResult => {
  ...
  const messages: Record<Exclude<TaskCancellationResult, { readonly ok: true }>["code"], string> = {
    ...
    active_task_review: `Cannot cancel Task ${taskId} while its Task Review is active.`,
  ...
  const help =
    result.code === "active_task_review"
      ? [`Run \`by task-review show ${result.reviewId}\` to inspect it.`]
