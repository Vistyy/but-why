...
import { decodeChangeCleanup, decodeChangeState } from "./sqliteChangeValueDecoders.js";
...
const readCleanupChange = (sql: SqlClient.SqlClient, changeId: string, idPrefix: string) =>
  Effect.gen(function* () {
    ...
    return yield* decodePersisted(operationName, () => {
      ...
      return { ...decodeSelectedChangeState(row, changeId, idPrefix), cleanup };
    });
  });
type StoredChangeStateRow = { readonly id: number; readonly state: unknown };
...
const recordCleanup = (
  ...
  Effect.gen(function* () {
    const selected = yield* readChangeState(sql, input.changeId, "record Change cleanup", idPrefix);
    ...
    if (changed) {
      yield* sql`UPDATE changes SET cleanup_state = ${input.cleanup.state}, cleanup_blocking_reason = ${input.cleanup.blockingReason}, updated_at = ${input.now} WHERE id = ${internalChangeId(input.changeId, idPrefix)}`;
    }
