...
const submitTask = (
  ...
  Effect.gen(function* () {
    ...
    const started = yield* dependencies.persistence.start({
      ...
      workspaceSetup: {
        tempRefName,
        worktreePath: expectedDisposableWorkspacePath(dependencies.mainCheckoutRoot, tempRefName),
      },
      now: input.now,
    });
    ...
    const workspace = yield* createWorkspace({
      ...
      runInWorkspace: (activeWorkspace) =>
        runTaskReviewPhases({
          ...
          dependencyEvidence: started.dependencyEvidence,
          baseCommit: head.commit,
  ...
  Effect.gen(function* () {
    ...
    if (!result.ok) {
      return {
        ...
        toolingFailure: taskReviewToolingFailureRecord({
          errorKind:
            result.failure.operationName === "decode_task_reviewer_output"
