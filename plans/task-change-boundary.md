...
Merged Change reconciliation closes the Change and marks the BY-274 Task Done in old state before the backup rename or fresh initialization, so later live steps are not a Task completion condition.
The live operator pauses state operations, reconciles the exact merged Change with the old executable, renames the old state directory as a dated low-value backup, initializes fresh state, and runs one basic CLI smoke check before post-baseline sequencing resumes.
