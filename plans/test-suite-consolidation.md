...
`docs/system-simplification-plan.md` supplies overlapping implementation-simplification context but does not authorize test changes.
...
This broad test seam is an important source of flexible setup, but `docs/system-simplification-plan.md` separately questions whether broad CLI injection objects preserve production seams used only by tests.
...
`docs/system-simplification-plan.md` already identifies verification setup caused by broad use-case objects, callback loaders, fixed-storage ports, construction-only Effect services, Layer topology, and test-only production flexibility.
