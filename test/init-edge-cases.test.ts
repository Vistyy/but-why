...
help[1]: Move the conflicting path aside before running init again.`);
  ...
  it.effect("records a repair when an existing state database lacks the init migration", () =>
    Effect.gen(function* () {
      const root = createGitRepo();
    ...
    `);
      ...
      expect(result.stdout).toContain(
        "updated[2]: <git-common-dir>/but-why/state.sqlite,.gitignore",
      );
  ...
  it.effect("moves historical Task Start worktree facts onto the Change", () =>
    Effect.gen(function* () {
      const root = createGitRepo();
      writeConfig(root);
  ...
  it.effect("adds Task approval state without losing existing Task-owned records", () =>
    Effect.gen(function* () {
      const root = createGitRepo();
    ...
    `);
      ...
      const migrationResult = yield* runByInProcessEffect(root, ["init", "--task-prefix", "BY"]);
      ...
      expect((yield* runByInProcessEffect(root, ["init", "--task-prefix", "BY"])).stdout).toContain(
        "status: unchanged",
      );
    }),
  );

  it.effect("aligns legacy reviewer phase storage", () =>
    Effect.gen(function* () {
      const root = createGitRepo();
      expect((yield* runByInProcessEffect(root, ["init", "--task-prefix", "BY"])).status).toBe(0);
      const statePath = sharedStatePath(root);
  ...
  ordinaryIt("records migration timestamps from BUT_WHY_NOW", () => {
    const root = createGitRepo();
