...
it.layer(NodeFileSystem.layer)((it) => {
  describe("Acceptance Review phase", () => {
    ...
    it.scoped("rejects a different workspace Candidate before reviewer launch", () =>
      Effect.gen(function* () {
        ...
        const fixture = acceptancePhaseFixture(
          { review },
          { observedHeadSha: "different-head-sha" },
        );
    ...
    it.scoped(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(firstResult.reviewerEvidence).toMatchObject({
            continuity: "fresh",
            reviewCalls: 1,
          });
    ...
    it.scoped(
      "restarts one unusable resumed Acceptance Reviewer Session for one fresh result",
      () =>
        Effect.gen(function* () {
          const sessions = new Map<string, ReviewerSessionRecord>();
          const sessionStore = memorySessionStore(sessions);
          const initial = acceptancePhaseFixture(
            {
              review: () =>
                Effect.succeed({ ...cleanReport, sessionReference: "superseded-session" }),
            },
            { sessionStore },
          );
          yield* initial.run();
  ...
  const unusedReviewerExecutor: ReviewerProcessExecutor = {
    execute: () =>
      Effect.die("Captured Reviewer Agent Runtime must not execute a reviewer process."),
  };
    ...
    return {
      ...
      run: () =>
        runAcceptanceReviewPhase({
          ...
          ...(options.blockerHistory === undefined
            ? {}
