      ...
      () =>
        Effect.gen(function* () {
          ...
          const review = vi.fn<ReviewerAgentRuntime<ReviewerOutput>["review"]>((input) => {
            if (input.reviewer === "zeta")
              return Effect.succeed(success([finding("Zeta Finding")]));
        ...
        Effect.gen(function* () {
          ...
          const successorReview = vi.fn<ReviewerAgentRuntime<ReviewerOutput>["review"]>((input) => {
            ...
            if (input.reviewer === "alpha")
              return Effect.succeed(success([], "alpha-fresh-session"));
            return Effect.succeed({
              ok: false as const,
    ...
    it.scoped(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            yield* Effect.suspend(() => validation.listRounds(durable.validationRunId)),
          ).toEqual([
          ...
          expect(
            yield* Effect.suspend(() => validation.listFindings(clean.validationRunId)),
          ).toEqual([]);
          expect(
            (yield* Effect.suspend(() => validation.listFindings(durable.validationRunId))).map(
              (item) => item.title,
          ...
          if (!laterSuccessor.ok)
            throw new Error(`Candidate capture failed: ${laterSuccessor.code}`);
