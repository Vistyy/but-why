      ...
      Effect.gen(function* () {
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            ...
            yield* sql`INSERT INTO tasks (id, numeric_id, title, description, state, cancel_reason, created_at, updated_at) VALUES
              ...
              ('BY-2', 2, 'Approved Task', 'Existing approved Task.', 'todo', NULL, '2026-08-01T09:01:00.000Z', '2026-08-01T09:01:00.000Z')`;
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            ...
            expect(
              yield* reviews.latestCompletedReviewForTask(publicTaskId("BY-2")),
            ).toBeUndefined();
            expect(yield* reviews.getTaskFact(publicTaskId("BY-2"))).toEqual({
              id: "BY-2",
              state: "todo",
            });
