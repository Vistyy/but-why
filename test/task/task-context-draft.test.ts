...
} from "../../src/repositoryRuntime/repositoryOperation.js";
...
const runtimeFor = (root: string): RepositoryOperationRuntime => {
...
const createTestTask = (root: string, description = "Original description") =>
  Effect.map(
    createTask(runtimeFor(root), { title: "Original title", description, now }),
    (result) => {
...
it.effect("creates and replaces Task Context drafts from current intent", () =>
  withRepository((root) =>
    Effect.gen(function* () {
      ...
      const first = yield* createTaskContextDraft(runtimeFor(root), publicTaskId("BY-1"));
      ...
      const replacement = yield* createTaskContextDraft(runtimeFor(root), publicTaskId("BY-1"));
...
it.effect("rejects malformed drafts before persistence and retains them", () =>
  withRepository((root) =>
    Effect.gen(function* () {
      yield* createTestTask(root);
      const draft = yield* createTaskContextDraft(runtimeFor(root), publicTaskId("BY-1"));
      if (draft === undefined) throw new Error("Expected Task Context draft");

      writeFileSync(draft.path, Buffer.from([0xff]));
      expect(
        yield* applyTaskContextDraft(runtimeFor(root), {
...
it.effect("reports cleanup failure after a successful Task Context persistence", () =>
  withRepository((root) =>
    Effect.gen(function* () {
      ...
      try {
        result = yield* applyTaskContextDraft(runtimeFor(root), {
          taskId: publicTaskId("BY-1"),
          now: later,
        });
      } finally {
