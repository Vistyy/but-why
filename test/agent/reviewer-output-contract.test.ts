...
describe("reviewer output contract", () => {
  ...
  const invalidOutputs: ReadonlyArray<readonly [name: string, output: unknown, path: string]> = [
    ...
    [
      ...
      { findings: [reviewerFinding({ confidence: 1 })] },
      "findings.0.confidence",
    ],
    ["empty Finding text", { findings: [reviewerFinding({ title: " " })] }, "findings.0.title"],
    [
      "non-relative file paths",
      { findings: [reviewerFinding({ files: ["../../src/cli.ts"] })] },
      "findings.0.files.0",
    ],
    [
      "malformed artifact refs",
      { findings: [reviewerFinding({ artifactRefs: ["logs.txt"] })] },
