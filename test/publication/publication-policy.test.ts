...
layer(publicationTemplateLayer)("Candidate publication", (it) => {
    ...
    withFixture((fixture) =>
      Effect.gen(function* () {
        ...
        const publication = openCandidatePublication({
          ...
          github: {
            ...
            updatePullRequest: () => ({
              ...
              observedRemoteHeadSha: "foreign-head",
            }),
          },
        });
        expect(yield* publication.publish(input(fixture))).toMatchObject({ ok: true });
        ...
        expect(
          ...
          ok: false,
          code: "publication_remote_mismatch",
          expectedRemoteHeadSha: next.captured.headSha,
          observedRemoteHeadSha: "foreign-head",
        });
        expect(yield* fixture.changes.reads.getChangeById(fixture.captured.changeId)).toMatchObject(
          {
  ...
  it.scoped("rejects a mismatched owned pull request identity during later recovery", () =>
    withFixture((fixture) =>
      Effect.gen(function* () {
        let branchHead = fixture.captured.headSha;
        ...
        const next = yield* nextCandidate(
          fixture,
          "Reject mismatched Candidate",
          "2026-07-22T10:05:00.000Z",
        );
        branchHead = next.captured.headSha;
        remote = { ...pullRequest(next.captured.headSha), headSha: "other-candidate" };
        expect(
          yield* publication.publish({
            ...input(fixture),
            candidateId: next.captured.candidateId,
  ...
  it.scoped("stops later recovery when an owned pull request fact is unavailable", () =>
    withFixture((fixture) =>
      Effect.gen(function* () {
        let branchHead = fixture.captured.headSha;
        let remote: GitHubPullRequest | undefined = pullRequest(branchHead);
  ...
  it.scoped("reopens and updates an exact closed-unmerged owned pull request", () =>
    withFixture((fixture) =>
      Effect.gen(function* () {
        let branchHead = fixture.captured.headSha;
        let remote: GitHubPullRequest = pullRequest(branchHead);
        ...
        const publication = openCandidatePublication({
          ...
          github: {
            ...
            updatePullRequest: (request) => {
              updates += 1;
              remote = {
                ...pullRequest(request.expectedHeadSha),
                state: "open",
              };
              return { ok: true as const, pullRequest: remote };
            },
          },
        });
        expect(yield* publication.publish(input(fixture))).toMatchObject({
        ...
        branchHead = next.captured.headSha;
        remote = { ...pullRequest(fixture.captured.headSha), state: "closed" };
        const reopened = yield* publication.publish({
          ...input(fixture),
          candidateId: next.captured.candidateId,
          validationRunId: next.validationRunId,
          now: "2026-07-25T15:45:00.000Z",
        });
