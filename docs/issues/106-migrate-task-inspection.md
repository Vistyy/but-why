...
- [ ] Change list sorts Changes from oldest to newest.
- [ ] Change list reports each open Change's age from `createdAt` to the command time without inferring inactivity.
...
- [ ] Structured output distinguishes an empty collection from unavailable detail.
