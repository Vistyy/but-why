...
- Focused tests retain real Git, Managed Worktree, SQLite concurrency, filesystem, and process coverage only where they prove an externally consequential adapter contract or reproduce a concrete prior regression.
- The routine suite verifies the packed-file manifest without defining a pre-publication installation workflow.
...
- User-approved: retain the packed-file manifest check and retire pre-publication local and global installation workflows and tests.
...
- User-approved: measured `just quality` runtime at or below 15 seconds and measured `just full-quality` runtime at or below 60 seconds are hard completion gates, although runtime warnings do not change command exit status.
- User-approved: maintain 10-second routine-quality and 30-second full-quality operating budgets, verify each through the median wall time of three consecutive runs in a clean locked-Nix checkout with dependencies installed and no competing heavy workload, and restore headroom in any change that exceeds a budget.
...
- `just test test/task/task-cli.test.ts test/task/task-cli-process.boundary.test.ts`
- `just test test/task/task-dependency-persistence.test.ts test/task/task-persistence-policy.test.ts test/repository/repository-storage.boundary.test.ts`
- `just test test/change/change-implement.boundary.test.ts`
- `just test test/change/change-reconciliation.test.ts`
- `just test test/repository/package-contents.test.ts`
- `just test`
