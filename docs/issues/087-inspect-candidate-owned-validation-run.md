# Inspect one Candidate-owned Validation Run
...
- Source capability: A caller retrieves complete detail for one Run UUID.
- Own Run, Attempt, input, Finding, failure, and Artifact detail with bounded content previews.
- This task owns only the behavior demonstrated through its primary verification seam.

## What to build

A caller retrieves complete detail for one Run UUID.
...
by validation-run show integration test.
