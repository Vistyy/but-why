# Run one configured Pi Specialist
...
- Source capability: One Specialist passes or blocks a checked Candidate.
- Own the Pi adapter, one Specialist policy, prompt, cancellation, one output retry, evidence, usage, and Findings.
- This task owns only the behavior demonstrated through its primary verification seam.

## What to build

One Specialist passes or blocks a checked Candidate.
...
Single-Specialist by validate test.
