# Recover interrupted repository work
...
- Source capability: Startup and periodic recovery find eligible work after lost wakes or crashes.
- Own durable recovery claims, idle exit, restart, and the code-owned circuit breaker after bounded recovery is exhausted.
- This task owns only the behavior demonstrated through its primary verification seam.

## What to build

Startup and periodic recovery find eligible work after lost wakes or crashes.
...
Lost-wake and crash test.

## Acceptance criteria

- [ ] Startup and periodic recovery find eligible work after lost wakes or crashes.
- [ ] But Why? code records Needs Input only after bounded recovery is exhausted, preserving a resumable checkpoint and recovery evidence.
...
- `docs/issues/102-watch-owned-prs-over-time.md`
