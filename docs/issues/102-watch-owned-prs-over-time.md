...
- Source capability: A bounded watcher rechecks pending PR facts without a generic polling subsystem.
- Own Effect scheduling, retry, timeout, cancellation, and compact progress facts.
- This task owns only the behavior demonstrated through its primary verification seam.

## What to build

A bounded watcher rechecks pending PR facts without a generic polling subsystem.
...
Controlled-schedule watcher test.
