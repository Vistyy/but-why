# Compare two reviewer suite reports
...
- Source capability: A maintainer sees regressions between two stored suite reports.
- Own comparison rules and regression output only.
- This task owns only the behavior demonstrated through its primary verification seam.

## What to build

A maintainer sees regressions between two stored suite reports.
...
- `docs/issues/121-run-one-calibrated-reviewer-suite.md`
