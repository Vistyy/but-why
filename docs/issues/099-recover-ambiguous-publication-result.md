# Recover an ambiguous publication result
...
- Source capability: A lost GitHub response cannot create a duplicate PR.
- Own Change marker lookup, branch and repository matching, remote-success recovery, and ambiguous-stop behavior.
- This task owns only the behavior demonstrated through its primary verification seam.

## What to build

A lost GitHub response cannot create a duplicate PR.
...
Lost-response fake GitHub test.
