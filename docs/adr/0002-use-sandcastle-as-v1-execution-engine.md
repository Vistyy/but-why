...
But Why? owns Validation Run-scoped temp validation refs and Validation Run-scoped tooling error diagnostics because Sandcastle takes a branch or ref as input and does not own But Why? Validation Run state.
Validation should pass Validation Run-owned temp refs to Sandcastle instead of running against the checked-out task branch.
